...
@Slf4j
@Component
public class WaitCommentState extends AbstractState {
    ...
    public boolean commentProduct(String orderSn, Enum<Constants.OrderState> currentOrderState) {
        return orderRepository.alterState(orderSn,currentOrderState, Constants.OrderState.HAVE_COMMENT);
    }

    @Override
    public boolean returnProduct(String orderSn, Enum<Constants.OrderState> currentOrderState) {
        return orderRepository.alterState(orderSn,currentOrderState, Constants.OrderState.AFTER_SALE);
    }
