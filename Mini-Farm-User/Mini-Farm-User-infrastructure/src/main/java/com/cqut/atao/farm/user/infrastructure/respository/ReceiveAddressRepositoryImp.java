...
public class ReceiveAddressRepositoryImp implements ReceiveAddressRepository {
    ...
    public List<ReceiveAddressRes> queryList(String userId) {
        ...
        List<ReceiveAddressRes> reqS = BeanUtil.convert(receiveAddressPOS, ReceiveAddressRes.class);
        return reqS;
    }
    ...
    public void saveReceiveAddress(ReceiveAddressReq req) {
        if (StringUtils.isNotBlank(req.getId()) && receiveAddressDao.exists(Wrappers.lambdaQuery(ReceiveAddress.class).eq(ReceiveAddress::getId,req.getId()))) {
            int res = receiveAddressDao.update(BeanUtil.convert(req, ReceiveAddress.class),Wrappers.lambdaQuery(ReceiveAddress.class).eq(ReceiveAddress::getId,req.getId()));
            Assert.isTrue(res > 0, () -> new ServiceException("修改收货地址失败"));
            return;
        }
        int res = receiveAddressDao.insert(BeanUtil.convert(req, ReceiveAddress.class));
    ...
    @Override
    public ReceiveAddressRes getDefaultAddress(Long userId) {
        ReceiveAddress receiveAddress = receiveAddressDao.selectOne(Wrappers.lambdaQuery(ReceiveAddress.class)
                .eq(ReceiveAddress::getUserId, userId)
                .eq(ReceiveAddress::getDefaultFlag, 1));
        return BeanUtil.convert(receiveAddress,ReceiveAddressRes.class);
    }
}
